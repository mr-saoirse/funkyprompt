...
{% hint style="info" %}
The guides below are for `funkyprompt` library usage You can also run a `cli` for some actions with poetry, in which case prefix all commands with `poetry run` \<COMMAND>
{% endhint %}
